...
class Collage < ActiveRecord::Base
  def torrents(order)
    ActiveRecord::Base.connection.execute("select a.Name, a.ArtistID, tg.ID, tg.CatalogueNumber, tg.Year, tg.RecordLabel, tg.Name from collages c join collages_torrents ct on c.ID = ct.CollageID join torrents_group tg on ct.GroupID = tg.ID join torrents_artists ta on tg.ID = ta.GroupID join artists_group a on ta.ArtistID = a.ArtistID where c.ID = #{self.ID} order by #{order}").to_a
  end
